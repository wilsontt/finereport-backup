...
| 5 | NAS >30MB 分卷上傳（掛載／smbclient 一律） | 降低大檔 SMB／CIFS 傳輸失敗 | ✅ 已實作 | `backupService.ts`、`lib/nasChunk.ts`；見 [NAS分卷上傳設計](./FineReport備份工具-NAS分卷上傳設計.md)、[PLAN](./FineReport備份工具-NAS分卷上傳-PLAN.md) |
...
> **後續調整（2026-06-11）**：實測 `schedule` 解壓近 5 萬個小檔仍嚴重拖慢流程，且打包本身已解決傳輸效能與深層路徑問題。
> 因此最終實作改為：**所有來源一律打包**，目的端**不再解壓**、僅保留各來源的 `.tgz`（額外效益：節省 NAS 空間），
> 移除檔案數門檻與 `downloadDir` 分支；目的目錄僅需建立第一層（如 `webroot/`、`mysqldata/`）。如需查看內容由使用者自行手動解壓。

...
let cachedSmbclientBin: string | null = null;

export function resolveSmbclientBin(): string {
 ...
 */
export function isSmbclientAvailable(): boolean {
...
const fullPath = (host: string, share: string, p: string) =>
  `\\\\${host}\\${share}\\${p.replace(/\//g, '\\')}`;
 ...
 */
async function findExistingNasMount(host: string, share: string): Promise<string | null> {
...
export async function mountNas(creds: NasCredentials, mountPoint: string): Promise<MountResult> {
  ...
  return new Promise((resolve, reject) => {
    if (isDarwin()) {
      ...
      proc.on('error', (e) => {
        reject(new Error(`ERR_NAS_MOUNT: ${(e as Error).message}`));
      });
    }
  });
}
 ...
 * 卸載 NAS 掛載點
 */
export async function unmountNas(mountPoint: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    proc.on('close', (code) => {
      if (code === 0) resolve();
      else reject(new Error(`ERR_NAS_UMOUNT: ${stderr}`));
    });
 ...
 */
async function verifyViaMount(creds: NasCredentials): Promise<{ ok: boolean; fullPath: string }> {
 ...
 */
async function verifyViaSmbclient(creds: NasCredentials): Promise<{ ok: boolean; fullPath: string }> {
  const host = creds.host.replace(/^smb:\/\//, '').trim();
 ...
 */
export async function verifyNas(creds: NasCredentials): Promise<{
  ok: boolean;
  fullPath: string;
}> {
  const timeoutPromise = new Promise<never>((_, reject) =>
    setTimeout(() => reject(new Error('ERR_NAS_TIMEOUT')), VERIFY_NAS_TIMEOUT_MS)
  );
 ...
 */
export async function listNasDirectory(creds: NasCredentials, path: string): Promise<{ name: string; isDir: boolean }[]> {
  const host = creds.host.replace(/^smb:\/\//, '').trim();
  const address = `//${host}/${creds.share}`;
 ...
 */
export async function createNasDirectory(

 ...
 * 是否應對封存檔進行分卷
 */
export function shouldChunkArchive(sizeBytes) {
    return sizeBytes > NAS_CHUNK_BYTES;
}
/**
 * split 前綴：`jar.tgz` → `jar.tgz.part`（後接 000、001…）
 */
export function partFilePrefix(tgzPathOrName) {
 ...
 * 分卷檔完整檔名（index 從 0）
 */
export function partFileName(tgzPathOrName, index) {

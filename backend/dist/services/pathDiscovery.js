 ...
 * 取得 auto 目錄下最新時間戳子目錄
 */
async function getLatestInAuto(creds, sudoPassword, relPath) {
    const fullPath = `${BACKUP_BASE}/${relPath}`;
    const { stdout, code } = await execWithSudo(creds, sudoPassword, `ls -1 "${fullPath}" 2>/dev/null | sort -r | head -1`);
 ...
 * 探索遠端目錄，解析 {latest}
 */
export async function discoverRemote(creds, sudoPassword, basePath = BACKUP_BASE) {
    const results = [];
...
//# sourceMappingURL=pathDiscovery.js.map
...
const FileWriteStream_1 = require("./stream/FileWriteStream");
...
class File extends events_1.EventEmitter {
    ...
    createWriteStream() {
        return new FileWriteStream_1.FileWriteStream(maxWriteChunkLength, this.writeChunk.bind(this));
    }
    async readChunk(initial, offset) {
    ...
    async read() {
        ...
        for (let index = 0; index < chunkCount; index++) {
            ...
            (await this.readChunk(index, offset)).copy(buffer, offset);
        }
        return buffer;
    }
    createReadStream() {
        const fileSize = Number(this.fileSize);
        return stream_1.Readable.from(async function* read() {
            const chunkCount = Math.ceil(fileSize / maxReadChunkLength);
            for (let index = 0; index < chunkCount; index++) {
                const offset = index * maxReadChunkLength;
                yield (await this.readChunk(index, offset));
            }
        }.bind(this)());
    }
    async exists(path) {

...
class Server {
    ...
    middlewares = [];
    constructor() {
        this.use((0, supportedProtocols_1.default)([protocolIds.smb, protocolIds.smb2]));
        const smb2RequestHandlerTypes = Object.keys(smb2RequestHandlers);
        for (const smb2RequestHandlerType of smb2RequestHandlerTypes) {
            const handler = (0, requestType_1.default)(protocolIds.smb2, PacketType_2.default[smb2RequestHandlerType], smb2RequestHandlers[smb2RequestHandlerType]);
            this.use(handler);
        }
        const smbRequestHandlerTypes = Object.keys(smbRequestHandlers);
        for (const smbRequestHandlerType of smbRequestHandlerTypes) {
            const handler = (0, requestType_1.default)(protocolIds.smb, PacketType_1.default[smbRequestHandlerType], smbRequestHandlers[smbRequestHandlerType]);
            this.use(handler);
        }
    ...
    async listen(port = 445) {
        ...
        this.startDate = (0, moment_timezone_1.default)().toDate();
        return this.server;
    }
    onConnection = (socket) => {
